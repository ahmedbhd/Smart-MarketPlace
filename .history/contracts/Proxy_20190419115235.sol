...
contract Proxy{
    ...
    mapping (uint256 => Purchase) private _pendingPurchases;
    uint256 private _pendingPurchasesNumber;
     mapping (uint256 => Purchase) private _confirmedPurchases;
    uint256 private _confirmedPurchasesNumber;
    constructor() public {
        _confirmedPurchasesNumber=_pendingPurchasesNumber =_housesNumber=0;
    }
    ...
    function addPurchase(uint256 _index,uint256 _houseIndex, address _bank, address _insurance, string memory _loan,string memory _date,string memory _forBank,string memory _forInsurance) public returns (bool){
        if (_index ==0){
            _pendingPurchasesNumber++;
            _pendingPurchases[_pendingPurchasesNumber] =new Purchase();
            _pendingPurchases[_pendingPurchasesNumber].setArguments(
                 _houses[_houseIndex].getOwner(),
                 _houses[_houseIndex].getBuyer(),
                 _bank,
                 _insurance,
                 _index,
                 _loan,
            ...
            _confirmedPurchasesNumber++;
            _confirmedPurchases[_confirmedPurchasesNumber] = _pendingPurchases[_index];
            delete _pendingPurchases[_index] ;
        }
        return true;
    }
    ...
    function getPurchasesNbr() public view returns (uint256){
        return _confirmedPurchasesNumber;
    }
    ...
    function getPurchaseAt(uint256 _index) public view returns(Purchase){
        return (_confirmedPurchases[_index]);
    }
