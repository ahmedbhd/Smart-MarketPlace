...
contract Proxy{
    ...
    function setPurchaseAsConfirmed(uint256 _purchaseIndex) public returns (bool){
        _purchases[_purchaseIndex].setBuyerConfirmation();
        uint256 _houseIndex = _purchases[_purchaseIndex].getHouseIndex();
        _setHouseAsConfirmed(_purchaseIndex,_houseIndex,_houses[_houseIndex].getOwner(),_houses[_houseIndex].getBuyer(),_houses[_houseIndex].getPrice(),_purchases[_purchaseIndex].getAdvance());
        return (true);
    }
