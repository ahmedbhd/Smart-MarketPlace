...
contract Proxy{
    ...
    function addPendingPurchase(uint256 _houseIndex, address _bank, address _insurance, string memory _loan,uint256 _date,string memory _forBank,string memory _forInsurance,string memory _advance) public returns (bool){
        _purchasesNumber++;
        _purchases[_purchasesNumber] =new Purchase();
        _purchases[_purchasesNumber].setArguments(
             _houses[_houseIndex].getOwner(),
