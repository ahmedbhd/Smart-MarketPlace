...
contract Proxy{
    ...
    function getMyPendingPurchasesNbr() public view returns (string memory){
        string memory _tab;
        string memory _j;
        uint256 _k;
        for (uint256 _i=1; _i<=_purchasesNumber; _i++) {
    ...
    function getMyInProgressPurchasesNbr() public view returns (string memory){
        string memory _tab;
        string memory _j;
        uint256 _k;
        for (uint256 _i=1; _i<=_purchasesNumber; _i++) {
    ...
    function getPurchaseLoanAt(uint256 _index) public view returns(string memory){
        return (_purchases[_index].getLoan());
    }
    ...
    function setPurchaseAsInProgress(uint256 _purchaseIndex) public returns (bool){
        ...
        require(msg.sender== _houses[_houseIndex].getBuyer());
        _purchases[_purchaseIndex].setBuyerConfirmation();
    ...
    function setPurchaseAsConfirmed(uint256 _purchaseIndex) public returns (bool){
        _purchases[_purchaseIndex].setBuyerConfirmation();
        uint256 _houseIndex = _purchases[_purchaseIndex].getHouseIndex();
        _setHouseAsConfirmed(_purchaseIndex,_houseIndex,_houses[_houseIndex].getOwner(),_houses[_houseIndex].getBuyer(),_houses[_houseIndex].getPrice());
        return (true);
    }
    function _setHouseAsConfirmed(uint256 _purchaseIndex ,uint256 _houseIndex , address _owner,address _buyer, uint256 _price) internal {
        require(address(0)!=msg.sender);
        emit Confirmed(_purchaseIndex,_houseIndex,_owner,_buyer,_price);
    }
    // the seller confirmes the purchase and notifies back the clearing house
