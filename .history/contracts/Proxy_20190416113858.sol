...
contract Proxy{
    ...
    mapping (uint256 => House) private _homes;
    ...
    function addHome(string memory _location,string memory _area,uint _rooms, uint256 _price,string memory _image) public returns (bool){
        _homesNumber++;
        _homes[_homesNumber] =new Home(_location,_area,_price,msg.sender,_rooms,_image);
        return true;
    }
   ...
   /*  function getPurchasesNbr() public view returns (uint256){
    ...
    function getMyInProgressHomes() public view returns (string memory){
    ...
    function setInProgress(uint256 _index, address _buyer) public returns (bool) {
        require(address(0)!=msg.sender);
        _homes[_index].setInProgress();
