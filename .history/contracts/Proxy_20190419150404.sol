...
contract Proxy{
    ...
    function addInProgressPurchase(uint256 _purchaseIndex,uint256 _houseIndex, address _buyer, address _bank, address _insurance, string memory _loan,string memory _date,string memory _forBank,string memory _forInsurance) public returns (bool){
        if (_purchaseIndex ==0){
            _purchasesNumber++;
    ...
    function addPendingPurchase(uint256 _houseIndex, address _bank, address _insurance, string memory _loan,string memory _date,string memory _forBank,string memory _forInsurance) public returns (bool){
        _purchasesNumber++;
        _purchases[_purchasesNumber] =new Purchase();
        _purchases[_purchasesNumber].setArguments(
             _houses[_houseIndex].getOwner(),
    ...
    function _setHouseAsConfirmed(uint256 _index) internal {
        require(address(0)!=msg.sender);
        emit Confirmed(_houses[_index].getOwner(),msg.sender,_houses[_index].getPrice());
    }
    // the seller confirmes the purchase and notifies back the clearing house
    function setHouseAsCanceled(uint256 _houseIndex, uint256 _purchaseIndex) public returns (bool) {
        require(address(0)!=msg.sender);
