...
contract Proxy{
    ...
    function addHouse(string memory _location,string memory _area,uint _rooms, uint256 _price,string memory _image) public returns (bool){
        _housesNumber++;
        _houses[_housesNumber] =new House();
        _houses[_housesNumber].setArguments(_location,_area,_rooms,_price,_image,msg.sender);
   ...
   /*  function getPurchasesNbr() public view returns (uint256){
    ...
    function getHouseAt(uint256 _index) public view returns(string memory, string memory, uint, uint, string memory, address ){
        string memory _s = _houses[_index].getRooms().uint2str();
        string memory _area = _houses[_index].getArea();
        _area = _area.concat(" / S+");
        _area = _area.concat(_s);
        return (
            _houses[_index].getLocation(),
            _area,
            _houses[_index].getPrice(),
            _houses[_index].getState(),
            _houses[_index].getImage(),
            _houses[_index].getOwner());
    }
    function getHouseBuyerAt(uint256 _index) public view returns( address ){
        return (_houses[_index].getBuyer());
    }
    ...
    function setHouseAtAsWanted (uint256 _index,uint256 _price) public {
        require(address(0)!=msg.sender);
        require(_price >= _houses[_index].getPrice());
