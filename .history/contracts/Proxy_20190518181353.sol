...
contract Proxy{
    ...
    function getHouseAt(uint256 _index) public view returns(string memory, string memory, uint, uint, address, address ){
        string memory _descLocationAreaRoomsReview;
        ...
        if (!_houses[_index].deleted){
            _descLocationAreaRoomsReview = _houses[_index].house.getDescLocationAreaRoomsReview();
    ...
    function unsellHouseAt(uint256 _index) public returns(bool){
        bool isHouseDeleted = _houses[_index].deleted;
        if (!isHouseDeleted){
    ...
    function getMyHouses(address _owner) public view returns (string memory){
        string memory _tab;
        string memory _j;
        uint256 _k;
        for(uint256 _i=1;_i<=_housesNumber;_i++){
            if (!_houses[_i].deleted)
                if (_houses[_i].house.getOwner()==_owner){
                    _k=_i;
                    _j=_k.uint2str();
                    _tab=_tab.concat(_j);
                    _tab=_tab.concat( ";");
