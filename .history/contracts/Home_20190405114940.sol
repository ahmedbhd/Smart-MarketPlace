...
contract Home {
    ...
    // the clearing house receives this event
    event Wanted(
        address  _buyer
    );
    ...
    // the seller receives this event to confirme the purchase
    event Pending(
        address  _buyer
    );
    ...
    function setConfirmed(address _buyer) public{
        require(_owner ==msg.sender);
        _state = "confirmed";
        emit Confirmed(msg.sender , _buyer);
    }
    // the clearing house receives this event to make the purchase
    event Confirmed(
        address  _owner,
        address _buyer
    );
