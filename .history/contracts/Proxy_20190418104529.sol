...
contract Proxy{
    ...
    function addHouse(string memory _location,string memory _area,string memory _rooms, uint256 _price,string memory _image) public returns (bool){
        _housesNumber++;
   ...
   /*  function getPurchasesNbr() public view returns (uint256){
    ...
    function getHouseAt(uint256 _index) public view returns(string memory, string memory, uint, uint, string memory, address ){
        uint _s;
        _s = _houses[_index].getRooms();
        string memory _area;
        _area = _houses[_index].getArea();
        _area = _area.concat("/");
        string memory _a = _s.uint2str();
        _area = _area.concat(_a);
        return (
