...
function initiateEvents() {
  ...
  confirmedEvent.watch(function(error, result) {
    if (!error) {
      ...
      let purchaseLoan = parseFloat(_paymentsTab[0]);
...
function upFrontPurchase(houseIndex, buyer) {
  ...
  proxyContract.addInProgressPurchase(0,houseIndex,buyer,0,0,_loanAdvanceMonthlyBankMonthlyInsurance,timeStamp,
   { from: clearingHouseAccount, gas: 3000000 }
  );
}
...
function confirmUpFrontPurchase(owner,buyer,price,houseIndex,purchaseIndex) {
  ...
  if (_buyerBalance >= price) {
    let _d = new Date();
    let _timeStamp = _d.getTime();
    let _history = "|"+buyer+"/"+_timeStamp+"/Bought";
    proxyContract.transferHouseFrom(houseIndex,owner,buyer,_history,{ from: clearingHouseAccount, gas: 3000000 },function(error, result) {
        if (!error) {
