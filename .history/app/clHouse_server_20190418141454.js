...
function initiateEvents(){
    ...
    wantedEvent.watch(function(error, result){
        if (!error)
        {
            ...
            proxyContract.setHouseInProgress(result.args._houseIndex,result.args._from, {from: clearingHouseAccount, gas:3000000 })
        } else {
            console.log(error);
        }
    });
        ...
        if (buyerBalance >= price){
            ...
            proxyContract.transferHouseFrom(owner,buyer,result.args._houseIndex,
                {from:clearingHouseAccount,gas:3000000 }, function (error, result) {
                    if (!error){
                        tokenContract.transferFrom(owner,buyer,price,{from:clearingHouseAccount,gas:3000000 });
