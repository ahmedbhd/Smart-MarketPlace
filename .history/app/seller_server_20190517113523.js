...
app.post("/addHouse", (req, res) => {
  ...
  let _history = _timeStamp+"/add"
  proxyContract.addHouse(location,area,req.body.price,
...
app.get("/getMyHouses", (req, res) => {
  ...
  let houses = [];
  let housesNbr = proxyContract.getMyHouses({
    from: sellerAccount,
    gas: 30000000
  });
  if (housesNbr != "") {
    ...
    tab.forEach(function(item) {
      ...
      let thisHouse = proxyContract.getHouseAt(item, {
        from: sellerAccount,
        gas: 3000000
      });
...
app.post("/setConfirmed", (req, res) => {
  ...
  res.json(
    proxyContract.setPurchaseAsConfirmed(
