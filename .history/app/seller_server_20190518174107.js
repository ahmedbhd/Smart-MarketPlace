...
app.post("/addHouse", (req, res) => {
  ...
  let _descLocationAreaRoomsReview = req.body.description + "|" + req.body.location+"|"+req.body.area + "|" + req.body.rooms+"|0/0";
  let _d = new Date();
  let _timeStamp = _d.getTime();
  let _history = sellerAccount+"/"+_timeStamp+"/Add"
  proxyContract.addHouse(_descLocationAreaRoomsReview,_history,req.body.price,sellerAccount,{ from: sellerAccount, gas: 3000000 },function(err, result) {
      if (!err) {
        console.log(result);
        res.json(result);
      } else {
