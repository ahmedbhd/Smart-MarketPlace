...
function initiateEvents(){
    ...
    confirmedEvent.watch(function(error, result){
        if (!error)
        {
            ...
            let purchaseLoanStr = proxyContract.getPurchaseLoanAt(purchaseIndex,{from: clearingHouseAccount, gas:3000000 });
            let purchaseLoan = parseFloat(purchaseLoanStr);
            if (purchaseLoan == 0){
                console.log("purchase up front")
                confirmUpFrontPurchase(owner, buyer, price, houseIndex);
            
            }else{
                console.log("purchase with loan "+buyerBalance+" "+price);
...
function upFrontPurchase(houseIndex,buyer){
    ...
    proxyContract.addInProgressPurchase(0,houseIndex,buyer,0,0,"0",timeStamp,"0","0","0",
...
function confirmUpFrontPurchase(owner, buyer, price, houseIndex){
    let currentBuyerBalance = tokenContract.balanceOf(buyer,{from: clearingHouseAccount, gas:3000000 });
    let buyerBalance = parseInt(currentBuyerBalance);
    if (buyerBalance >= price){
...
function confirmPurchaseWithLoan(owner, buyer, price, houseIndex,purchaseIndex){
    let currentBuyerBalance = tokenContract.balanceOf(buyer,{from: clearingHouseAccount, gas:3000000 });
    let buyerBalance = parseInt(currentBuyerBalance);
    ...
    let advanceStr =  purchase.getAdvance({from:sellerAccount,gas:3000000 });
    ...
    if (buyerBalance>=advance){
        proxyContract.transferHouseFrom(houseIndex,owner,buyer,
            {from:clearingHouseAccount,gas:3000000 }, function (error, result) {
                if (!error){
                    tokenContract.transfer(bank,advance,{from:buyer,gas:3000000 }, function (error, result) {
                        if (error){
                            proxyContract.revertPurchaseOf(houseIndex,{from:buyer,gas:3000000 }, function (error, result) {
                                if (!error){
                                    console.log("purchase revert done ");
                            ...
                            console.log("transfer success");
                            tokenContract.transfer(owner,price,{from:bank,gas:3000000 });
                        }});
                    console.log("transfer success");
                }else {
                    console.log("transfer error "+error);
