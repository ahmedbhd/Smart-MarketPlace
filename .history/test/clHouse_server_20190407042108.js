...
io.on('connection', function(socket){
    console.log("socket to be emitted "+proxyContractAddress);
    socket.emit("proxyAddress",proxyContractAddress);
});
io.listen(8088);
...
function deploySCToken (){
    ...
    let MyContractDeployment = web3.eth.contract(JSON.parse(compiledContracts['STToken.sol:STToken'].interface));
    tokenContract = MyContractDeployment.new(clearingHouseAccount, {
        from:clearingHouseAccount,
        data:'0x'+compiledContracts['STToken.sol:STToken'].bytecode,
        gas:5000000}, function(err, MyContractDeployment){
        if(!err) {
            if(!MyContractDeployment.address) {
                console.log("The hash of the transaction : "+MyContractDeployment.transactionHash) // The hash of the transaction, which deploys the contract
...
function initiateEvents(){
    ...
    {
        console.log(result.args._homeIndex);
        let homeAddress = proxyContract.getHomeAt(result.args._homeIndex,{from:clearingHouseAccount,
            gas:3000000 });
        console.log("home address :"+homeAddress)
        let thisHome = home.at(homeAddress);
        let owner = thisHome.getOwner({from:clearingHouseAccount,gas:3000000 })
        let buyer = thisHome.getBuyer({from:clearingHouseAccount,gas:3000000 })
        thisHome.transfer(owner,buyer,{from:clearingHouseAccount,gas:3000000 })
    } else {
        console.log(error);
    }
});
